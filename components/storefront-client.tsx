...
export function StorefrontClient({
  ...
  // The fixed promo bar would escape the dashboard's phone mockup, so previews skip it.
  const showPromoBar = footerVisible && !previewMode
  ...
  // Badges are only ever derived from real data: creator pins, actual clicks, real dates.
  const trendingIds = useMemo(
    () =>
      new Set(
        [...products]
          .filter((product) => (product.outbound7d ?? 0) >= 3)
          .sort((a, b) => (b.outbound7d ?? 0) - (a.outbound7d ?? 0))
          .slice(0, 3)
  ...
  const mobileFilteredProducts = useMemo(
    () =>
      latestProducts.filter(
        (product) =>
          (selectedCategories.length === 0 || selectedCategories.includes(normalizeCategory(product.category))) &&
          matchesProductQuery(product, deferredQuery),
      ),
    [deferredQuery, latestProducts, selectedCategories],
  )
  ...
  const desktopFilteredProducts = useMemo(() => {
    const list = normalizedProducts.filter(
      (product) =>
        (selectedCategories.length === 0 || selectedCategories.includes(normalizeCategory(product.category))) &&
        matchesProductQuery(product, deferredQuery),
    )
  ...
  const bannerText =
    user.storeBio?.trim() ||
    "Spotted something in my posts? Search its number (like #12) in the Shop tab. I may earn a small commission."
  ...
  const showcaseProducts = useMemo(() => {
    const pool = Array.isArray(mostBoughtProducts) && mostBoughtProducts.length > 0 ? mostBoughtProducts : featuredProducts
  ...
  }, [featuredProducts, mostBoughtProducts, source, sessionId, currentPath])
  ...
  useEffect(() => {
    ...
    const linkedQuery = (params.get("q") || "").slice(0, 80)
    const linkedCategory = (params.get("category") || "").trim().toLowerCase()
    const matchedCategory = products
      .map((product) => normalizeCategory(product.category))
      .find((category) => category.toLowerCase() === linkedCategory)
    if (linkedQuery) setQuery(linkedQuery)
    if (matchedCategory) setSelectedCategories([matchedCategory])
    if (linkedQuery || matchedCategory) setActiveTab("shop")
  ...
  useEffect(() => {
    if (disableTracking || !sessionId || !user._id) return
  ...
  }, [disableTracking, leadPopupStorageKey, sessionId, user._id])
  ...
  function renderCategoryChips() {
    if (categoryCounts.length < 2) return null
    const chipClass = (active: boolean) =>
      cn(
        "shrink-0 border px-3 py-1.5 text-[12px] font-semibold transition-colors",
        buttonRadiusClass,
        active
          ? "border-transparent"
          : isDarkMode
    ...
    const activeStyle = { backgroundColor: primaryColor, color: primaryTextColor }
    return (
      <div className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-1 [scrollbar-width:none]" role="group" aria-label="Filter by category">
        <button
          type="button"
          onClick={() => startTransition(() => setSelectedCategories([]))}
          aria-pressed={selectedCategories.length === 0}
          className={chipClass(selectedCategories.length === 0)}
          style={selectedCategories.length === 0 ? activeStyle : undefined}
        >
          All
        </button>
  ...
  function renderProductCard(product: StorefrontProduct, index: number, variant: "mobile" | "desktop") {
    ...
    return (
      <a
        ...
        aria-label={`${product.productNumber ? `#${product.productNumber} ` : ""}${product.title}${price ? `, ${price}` : ""} (opens retailer in a new tab)`}
        className={cn(
          "app-reveal app-surface content-auto group flex flex-col overflow-hidden rounded-xl border shadow-none transition-transform focus-visible:outline-none focus-visible:ring-2",
          variant === "desktop" && "hover:-translate-y-0.5",
          isDarkMode ? "border-slate-700 bg-slate-900 focus-visible:ring-slate-400" : "border-white/65 bg-white focus-visible:ring-slate-400",
        )}
      ...
      >
        <div
          className={cn(
            ...
            variant === "desktop" ? "aspect-[4/5]" : "aspect-square",
        ...
        >
          <Image
            ...
            fill
            unoptimized
            sizes={variant === "desktop" ? "(max-width: 1280px) 20vw, 220px" : "(max-width: 768px) 50vw, 160px"}
          />
          <div className="absolute left-1.5 top-1.5 flex gap-1">
            ...
            {product.isPinned ? (
              <span className="rounded-md bg-amber-400 px-1.5 py-0.5 text-[10px] font-bold text-amber-950">★ Top pick</span>
            ) : trendingIds.has(product._id) ? (
              <span className="rounded-md bg-rose-500 px-1.5 py-0.5 text-[10px] font-bold text-white">Trending</span>
            ) : renderedAt - product.createdAt < 7 * 24 * 60 * 60 * 1000 ? (
              <span className="rounded-md bg-emerald-500 px-1.5 py-0.5 text-[10px] font-bold text-white">New</span>
            ) : null}
          </div>
        </div>
        <div className={cn("flex flex-1 flex-col gap-0.5 px-2.5 py-2", isDarkMode ? "text-slate-100" : "text-slate-800")}>
          <p className={cn("line-clamp-2 font-semibold leading-snug", variant === "desktop" ? "text-[13px]" : "text-[12px]")}>
            {product.title}
          </p>
          <div className="mt-auto flex items-center justify-between gap-2 pt-0.5">
            <span className="truncate text-[12px] font-bold" style={{ color: isDarkMode ? undefined : accentColor }}>
              {price}
            </span>
            <ArrowUpRight className={cn("h-3.5 w-3.5 shrink-0", isDarkMode ? "text-slate-400" : "text-slate-400")} aria-hidden />
          </div>
        </div>
      </a>
    )
    ...
    return (
      <div className={cn("overflow-hidden rounded-2xl p-3 shadow-none", cardTone)}>
        <div className="grid grid-cols-3 gap-1">
          {showcaseProducts.map((product, index) => (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer sponsored"
              onClick={() => trackEvent("product_card_click", product.id)}
              aria-label={`${product.title} (opens retailer in a new tab)`}
              className={cn(
                "relative block overflow-hidden rounded-lg bg-white",
                index === 0 && showcaseProducts.length >= 3 ? "col-span-2 row-span-2" : "aspect-square",
              )}
            >
              <Image
                src={product.image}
                ...
                fill
                unoptimized
                sizes={index === 0 ? "(max-width: 768px) 66vw, 280px" : "(max-width: 768px) 33vw, 120px"}
              />
            </a>
          ))}
        </div>
        ...
        >
          ...
          <p className={cn("mt-0.5 text-xs", captionClass)}>
            {latestProducts.length} product{latestProducts.length === 1 ? "" : "s"}
          </p>
        </button>
      </div>
    )
  ...
  return (
    <div className="min-h-screen">
      ...
      >
        {footerVisible ? (
          <div className="absolute left-3 top-3 z-20">
            <button
              ...
              aria-label="Create your own store on Linkstore"
            >
        ...
        <main className={cn("relative z-10 mx-auto w-full max-w-md px-2 pt-7", showPromoBar ? "pb-20" : "pb-6")}>
          ...
          {activeTab === "links" ? (
              ...
              {socialItems.map((item, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <ArrowUpRight className={cn("h-4 w-4", isDarkMode ? "text-slate-400" : "text-slate-500")} aria-hidden />
                  </a>
                )
              })}
            ...
            <section className="mt-4" aria-busy={query !== deferredQuery}>
              ...
              <div className="mt-3">{renderCategoryChips()}</div>

              {mobileFilteredProducts.length > 0 ? (
                <div className="mt-3 grid grid-cols-2 gap-2.5">
                  {mobileFilteredProducts.map((product, index) => renderProductCard(product, index, "mobile"))}
                </div>
              ) : (
                <div
                  className={cn(
                ...
                >
                  ...
                  <p className={cn("mt-1 text-xs", isDarkMode ? "text-slate-400" : "text-slate-500")}>
                    {products.length === 0 ? "This store has not added products yet." : "Try another search or category."}
                  </p>
                </div>
              )}
            </section>
      ...
      <Dialog open={isShareOpen} onOpenChange={setIsShareOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>Send {displayName}&apos;s picks to a friend.</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="break-all rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-700">
              {shareUrl}
            </div>
            <Button
              type="button"
      ...
      >
        ...
        <main className="w-full space-y-3 px-2 pt-2 sm:space-y-4 sm:px-3 sm:pt-3 md:px-4 lg:px-5" aria-busy={query !== deferredQuery}>
          <section id="catalog">
            ...
            <div className="mb-4">{renderCategoryChips()}</div>
              ...
              groupedProducts.map((section) => (
                <div key={section.category} className="content-auto mb-4 last:mb-0">
                  ...
                  <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 sm:gap-2.5 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
                    {section.products.map((product, productIndex) => renderProductCard(product, productIndex, "desktop"))}
                  </div>
                </div>
              ))
            ) : (
